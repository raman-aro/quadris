...
std::vector<int> JBlock::getOldCoordinates() {
	return oldCoordinates;
}
std::vector<int> JBlock::getNewCoordinates() {
	return newCoordinates;
}
void JBlock::changeOldCoordinates(std::vector<int> changed) {
...
int JBlock::getOrientation() {
	return orientation;
}

std::vector<int> JBlock::getPointers () {
	return newCoordinates;
}
	...
	if (orientation == 1) {
		...
		if (brdptr->cellCheck((x - 1), y) &&
			...
			std::vector<int> newvec {(x - 1), (y + 1), (x - 1), y,
			                         x, y, (x + 1), y};
			//notify observers
			brdptr->notify(old, newvec, make_shared<JBlock>(*this));
		...
		if ((y + 1) <= 10 &&
			...
			//create vector of new positions
			std::vector<int> newvec {(x + 2), y + 1, (x + 1), (y + 1),
			                         (x + 1), y, (x + 1), (y - 1)};
			//notify observers
			brdptr->notify(old, newvec, make_shared<JBlock>(*this));
			//change CellPtrs to new positions
		...
		if (brdptr->cellCheck(x, y - 1) &&
		        ...
		        brdptr->cellCheck((x - 2), (y - 1))) {
			//create vector of new positions
			std::vector<int> newvec {x, y - 2, x, (y - 1),
			                         (x - 1), (y - 1), (x - 2), (y - 1)};
			//notify observers
			brdptr->notify(old, newvec, make_shared<JBlock>(*this));
			//change CellPtrs to new positions
		...
		if ((y + 2) <= 10 &&
		        ...
		        brdptr->cellCheck(x, (y + 2))) {
			//create vector of new positions
			std::vector<int> newvec {(x - 1), y, x, y, x, (y + 1), x, (y + 2)};
			//notify observers
			brdptr->notify(old, newvec, make_shared<JBlock>(*this));
			//change CellPtrs to new positions
			changeCellPTRS(newvec);
...
//uses cellptr1 for reference
	...
	if (orientation == 1) {
		...
		if ((y + 1) <= 10 &&
		        ...
		        brdptr->cellCheck((x + 2), (y + 1))) {
			//create vector of new positions
			std::vector<int> newvec {(x + 1), y - 1, (x + 2), (y - 1),
			                         (x + 2), y, (x + 2), (y + 1)};
			//notify observers
			brdptr->notify(old, newvec, make_shared<JBlock>(*this));
			//change CellPtrs to new positions
		...
		// checking that rotation does not go off the board and
		//  that potential spaces are empty
		if (brdptr->cellCheck(x, y - 2) &&
		        brdptr->cellCheck(x - 2, (y - 2)) &&
		        brdptr->cellCheck((x - 2), (y - 1))) {
			//create vector of new positions
			std::vector<int> newvec {x - 2, y - 1, x - 2, (y - 2),
			                         (x - 1), (y - 2), x, (y - 2)};
			//notify observers
			brdptr->notify(old, newvec, make_shared<JBlock>(*this));
			//change CellPtrs to new positions
		...
		if ((y + 2) <= 10 &&
		        ...
		        brdptr->cellCheck(x, (y + 2))) {
			//create vector of new positions
			std::vector<int> newvec {x, y + 2, x - 1, y + 2,
			                         x - 1, (y + 1), x - 1, y};
			//notify observers
			brdptr->notify(old, newvec, make_shared<JBlock>(*this));
			//change CellPtrs to new positions
...
void JBlock::changeCellPTRS(std::vector<int> vec) {
	newCoordinates = vec;
}
...
//uses cellptr1 for reference
	...
	if (orientation == 1) {
		...
		if ((y + 1) <= 10 &&
		        ...
		        brdptr->cellCheck((x + 2), (y))) {
			//create vector of new positions
			std::vector<int> newvec {x, y + 1, x, y, x + 1, y, x + 2, y};
			//notify observers
			brdptr->notify(old, newvec, make_shared<JBlock>(*this));
			//change CellPtrs to new positions
			changeCellPTRS(newvec);
...
void JBlock::drop() {
	while (down()) {}
	brdptr->printGraphicsDisplay(make_shared<JBlock>(*this));
...
void JBlock::changeLevel(int new_level) {
	level = new_level;
}

int JBlock::getActiveCells() {
	return activeCells;
}

void JBlock::changeActiveCells(int destroyed) {
	activeCells -= destroyed;
}

string JBlock::getLetter() {
	return letter;
}

void JBlock::changeLetter(string l) {
	letter = l;
}

std::vector<int> JBlock::getBottomLeft() {
	int x, y;
	y = newCoordinates[1];
	x = newCoordinates[0];
